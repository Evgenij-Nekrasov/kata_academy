...
import { FooterNavigation, cardPath } from '@/constants/pages/mainPage';
...
const FooterLinks = () => {
  return (
    <footer className="flex justify-between text-[20px] max-m:text-[17px] max-ml:text-[15px]  flex-wrap max-lg:grid max-lg:grid-cols-2 max-lg:ml-[100px] max-md:ml-[60px] max-sm:ml-[35px] max-m:ml-[0px] max-lg:gap-y-[25px]  gap-x-[25px] max-md:gap-x-[40px] max-ml:gap-x-[0px] pt-[80px] pb-[50px]">
      <ul className="listStyle">
        {FooterNavigation.map((navItem, index) => (
          <Link key={index} href={`https://kata.academy/${navItem.src}`}>
            <li className=" mb-2">{navItem.title}</li>
          </Link>
        ))}
      </ul>
      <ul className="listStyle">
        ...
        {cardPath.map((navItem, index) => (
          <Link key={index} href={`https://kata.academy/${navItem.alt}`}>
            <li className=" mb-2">{navItem.name}</li>
          </Link>
        ))}
      </ul>
      <ul className="listStyle">
        ...
        <ul className="flex gap-[10px] cursor-pointer">
          {socialLinks.map((socialItem, index) => (
            ...
            >
              <li
                key={index}
                className={`${socialItem.bg} w-[32px] h-[32px] max-ml:flex   max-ml:flex-wrap rounded-full flex  justify-center`}
              >
                <Image
